...
pub fn create_router(services: Arc<CloudflareServices>) -> Router {
    Router::new()
        ...
        .route(
            ...
            post(security::toggle_under_attack),
        )
        .route("/security/waf/rules", get(security::list_waf_rules))
        .route(
            "/security/firewall/rules",
            get(security::list_firewall_rules),
        )
        .route(
            "/security/firewall/rules",
            post(security::create_firewall_rule),
        )
        .route(
            "/security/firewall/rules/:id",
            delete(security::delete_firewall_rule),
        )
        .route(
            "/security/ip-access/rules",
            get(security::list_ip_access_rules),
        )
        .route("/security/ip-access/block", post(security::block_ip))
        .route("/security/ip-access/allow", post(security::allow_ip))
        .route(
            "/security/ip-access/rules/:id",
            delete(security::delete_ip_access_rule),
        )
        // Page Rules routes
        ...
        .route(
            "/workers/kv/namespaces/:id",
            delete(workers::delete_kv_namespace),
        )
        .route("/workers/kv/:namespace/keys", get(workers::list_kv_keys))
        .route(
            "/workers/kv/:namespace/values/:key",
            get(workers::get_kv_value),
        )
        .route(
            "/workers/kv/:namespace/values/:key",
            put(workers::set_kv_value),
        )
        .route(
            "/workers/kv/:namespace/values/:key",
            delete(workers::delete_kv_value),
        )
        // R2 Storage routes
        ...
        .route(
            "/stream/live-inputs/:id/urls",
            get(stream::get_live_input_urls),
        )
        // D1 Database routes
        ...
        .route(
            "/d1/databases/:id/tables/:table/schema",
            get(d1::get_table_schema),
        )
        // Analytics routes
